...
const CardAyat = lazy(() => import("../components/CardAyat"));
import HtmlToTeks from "../utils/HtmlToTeks";
import Title from "../utils/Title";
import SkeletonSurahCard from "../utils/SkeletonSurahCard";
      ...
      <div className="bg-slate-600 rounded-lg p-2">
        <div className="flex space-x-1 justify-start items-center">
          <div className="border-2 rounded-full h-10 w-10 flex justify-center items-center">
            {detailSurahData.nomor}
          </div>
          <div className="font-amiri text-5xl">{detailSurahData.nama}</div>
          <div className="italic text-sm ">
            {detailSurahData.namaLatin} / {detailSurahData.arti}
          </div>
        </div>
        <div>
          <HtmlToTeks teks={detailSurahData.deskripsi} />
        </div>
      </div>
      <Suspense fallback={<SkeletonSurahCard />}>
        {dataAyat.map((e, i) => (
          <CardAyat {...e} key={i} />
        ))}
      </Suspense>
